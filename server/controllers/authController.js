...
const createUser = async (req, res) => {
    try {
      ...
      if (!emailRegex.test(email)) {
        return res
          .status(400)
          .json({ status: "error", message: "Invalid email format", data: null });
      }
      ...
      let finalAdminRole = adminRole;
  
      if (userCount === 0) {
        finalRole = "superadmin";
        finalAdminRole = "superadmin";
      ...
      const user = new User({
        ...
        role: finalRole,
        adminRole: finalRole === "admin" || finalRole === "superadmin" ? finalAdminRole : undefined,
      ...
      console.log(`User created: ${email} with role: ${finalRole}`);
...
const forgotPassword = async (req, res) => {
  try {
    ...
    if (!email) {
      return res
        .status(400)
        .json({ status: "error", message: "Email is required", data: null });
    }
